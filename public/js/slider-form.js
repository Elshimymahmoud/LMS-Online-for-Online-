...
$(document).on('click', '#btn-submit', function (e) {
    ...
    if (validateForm($(this))) {
        ...
        dataJson.hero_text_ar =  hero_text_ar;
        dataJson.sub_text_ar=  sub_text_ar;
        ...
        }else if($('select[name="widget"]').val() == 3){
            dataJson.widget = {type:3};
        }
        ...
        if($('.button-wrapper').length > 0){
            ...
            $('.button-container .button-wrapper').each(function (key, value) {
                ...
                var label_ar = $(value).find('input[name="button_label_ar"]').val();
                var link =  $(value).find('input[name="button_link"]').val();
                buttons.push({label:label,label_ar:label_ar,link:link});
            })
