...
class BlogController extends Controller
{
    ...
    public function getIndex(Request $request)
    {
        ...
        if ($request->slug != "") {
            ...
            // return view($this->path.'.blogs.blog-single1', compact('blog','previous','next','popular_tags','categories','related_news'));
        
            return view($this->path.'.blogs.blog-single1-new', compact('blog','previous','next','popular_tags','categories','related_news'));
        }


        $blogs = Blog::has('category')->OrderBy('created_at','desc')->paginate(6);
        return view($this->path.'.blogs.index2-new',
