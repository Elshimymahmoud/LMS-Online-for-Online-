...
class ConfigController extends Controller
{
    ...
    public function getGeneralSettings()
    {
        ...
        try {
            $apiKey = config('google_font_api_key');
            $response = $client->request('GET', 'https://www.googleapis.com/webfonts/v1/webfonts', [
                'query' => [
                    'key' => $apiKey
                ]
            ]);

            $fonts = json_decode($response->getBody()->getContents());
        ...
        return view('backend.settings.general', compact('sections', 'footer_data','app_locales','api_clients', 'font_names'));
    }
// ***********
// *************
public function getlandingSettings()
    {
        ...
        return view('backend.settings.landing', compact('Landing_color'));
    }
    public function savelandingSettings(Request $request)
    {
        $request = $this->saveFiles($request);
        $Landing_id = Landing_color::find(1);
        if(!$Landing_id){
        Landing_color::create(array_merge($request->all()));
        }
        else{ 
            ...
            $about_color=$request->about_color;
            $courses_color=$request->courses_color;
            $speaker_color=$request->speaker_color;
            $blog_color=$request->blog_color;
            $sponser_color=$request->sponser_color;

            

            Landing_color::where("id", 1)->update(['body_color'=>$body_color,'heading_color'=>$heading_color ,
            'paragraph_color'=>$paragraph_color ,
            'icon_color'=>$icon_color ,'about_color'=>$about_color ,
            'courses_color'=>$courses_color ,'speaker_color'=>$speaker_color ,
            'blog_color'=>$blog_color,'sponser_color'=>$sponser_color ]);
...
// **********
    public function saveGeneralSettings(Request $request)
    {
        ...
        if ($request->get('myfatoorah__active') == null) {
            $requests['myfatoorah__active'] = 0;
        }
        if ($request->get('tabby__active') == null){
            $requests['tabby__active'] = 0;
        }
        if ($request->get('jeel__active') == null){
            $requests['jeel__active'] = 0;
        }
