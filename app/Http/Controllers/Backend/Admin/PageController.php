...
class PageController extends Controller
{
    ...
    public function store(StorePagesRequest $request)
    {
        ...
        if ($request->page_doc) {
            $this->validate($request, [
                'page_doc' => 'required|file|mimes:pdf|max:2048', // Only allow PDFs up to 2MB
            ]);

            // Get the uploaded file
            $file = $request->file('page_doc');

            // Create a unique filename using the original name and current timestamp
            $filename = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME); // Get the name without extension
            $extension = $file->getClientOriginalExtension(); // Get the extension
            $fullFilename = $filename . '-' . time() . '.' . $extension; // Combine to create the full filename

            // Define the destination path within the 'public/storage/pages' directory
        ...
        $message_en = $request->get('content');
        $dom_en = new \DOMDocument();
        $dom_en->loadHtml(mb_convert_encoding($message_en,  'HTML-ENTITIES', 'UTF-8'), LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
        ...
        $page->content_en = $dom_en->saveHTML();
    ...
    public function update(UpdatePagesRequest $request, $id)
    {
        ...
        if ($request->page_doc) {
            ...
            if ($page->file) {
                $existingFilePath = public_path('storage/pages/' . $page->file);
        ...
        $message_en = $request->get('content_en');
        $dom_en = new \DOMDocument();
        $dom_en->loadHtml(mb_convert_encoding($message_en,  'HTML-ENTITIES', 'UTF-8'), LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
        $images = $dom->getElementsByTagName('img');
