...
trait UserAttribute
{
    ...
    public function getActivateButtonAttribute()
    {
        ...
        $user->save();
        return'<a href="'.route('activate',[$this->id,$code,true]).'"  mc:edit class="dropdown-item"><span>'.__('buttons.backend.access.users.activate_user').'</span></a>';

        // return '<a href="'.route('admin.auth.user.restore', $this).'" name="confirm_item" class="btn btn-info"><i class="fas fa-sync" data-toggle="tooltip" data-placement="top" title="'.__('buttons.backend.access.users.restore_user').'"></i></a> ';
    }
    public function addUserToCourse()
    {
        return '<a href="'.route('admin.auth.user.addToCourse', $this).'" name="" class="dropdown-item">'.__('buttons.backend.access.users.addToCourse').  '</a> ';
    }
    public function addUserToGroup()
    {
        return '<a href="'.route('admin.auth.user.addToGroup', $this).'" name="" class="dropdown-item">'.__('buttons.backend.access.users.addToGroup').  '</a> ';
    }
    /**
     * @return string
     */
    	...
    	<div class="btn-group" role="group" aria-label="'.__('labels.backend.access.users.user_actions').'">
		  ...
		  <div class="btn-group btn-group-sm" role="group">
			...
			<div class="dropdown-menu" aria-labelledby="userActions">
              ...
              '.$this->getActivateButtonAttribute().'
              '.$this->addUserToGroup().'
			</div>
		  </div>
		</div>';
