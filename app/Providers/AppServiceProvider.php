...
//        }
        ...
        view()->composer(['frontend.layouts.*', 'frontend-rtl.layouts.*'], function ($view) {
            ...
            $featured_categories = Category::withoutGlobalScope('filter')->get();
            $all_courses = Course::withoutGlobalScope('filter')->where('published', '=', 1)->whereHas('category')->get();
            $view->with(compact('custom_menus', 'max_depth','menu_name','featured_categories','all_courses'));
        });
        ...
        view()->composer(['frontend.*', 'frontend-rtl.*'], function ($view) {
            ...
            $view->with(compact('global_featured_course','featured_courses','featured_categories'));
        });
