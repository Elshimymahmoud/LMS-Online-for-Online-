...
@if(config('app.display_type') == 'rtl' || (session()->has('display_type') && session('display_type') == 'rtl'))
    ...
    @else
        ...
        @if(!empty($blog))
            @include('frontend.layouts.courses-blog-header')
        @elseif(!empty($rates))
            @include('frontend.layouts.courses-rate-header')
        @else
            @include('frontend.layouts.courses-header')
        @endif


        <div class="app-body" style="margin-top: 112px">
            @include('backend.includes.courses_sidebar')
