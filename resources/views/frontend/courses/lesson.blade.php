...
@push('after-styles')
    ...
    <style>
        ...
        .options-list li.correct:before {
            ...
            color: green;
            @if (session('locale') == 'ar')margin-right: -1.3em;
        @else margin-left: -1.3em;
            @endif
                    ...
                    <div class="course-details-item border-bottom-0 mb-0">
                        ...
                        @if ($lesson->mediaPDF)
                            ...
                            @else
                                ...
                                <div class="course-single-text mb-5">
                                    <iframe src="{{ asset('storage/uploads/' . $lesson->mediaPDF->name . '#toolbar=0') }}"
                                        ...
                                        <ul class="options-list pl-4">
                                            ...
                                            @else
                                                @switch($question->question_type)
                                                        @case("paragraph")
                                                        <textarea type="text" class="form-control editor"   placeholder="paragraph" style="height:100px" name="questions[{{ $question->id }}]">{{ @test_res($test_result->id, $question->id)->option_id }}</textarea>
                                                        @break
                                                        @case("short_answer")
                                                        <input type="text" class="form-control" value="{{ @test_res($test_result->id, $question->id)->option_id }}" placeholder="short_answer" name="questions[{{ $question->id }}]" />
                                                        @break
                                                        @case("date")
                                                        <input type="text" class="form-control datepicker" value="{{ @test_res($test_result->id, $question->id)->option_id }}" placeholder="date" name="questions[{{ $question->id }}]" />
                                                        @break
                                                        @case("time")
                                                        <input type="text" class="form-control timepicker" value="{{ @test_res($test_result->id, $question->id)->option_id }}"  placeholder="time" name="questions[{{ $question->id }}]" />
                                                        @break
                                                        @case("file_upload")
                                        ...
                                        <div class="row main-content" style="margin-bottom: 60px;">
                                            <div class="col-4 offset-4">
                                                {{-- <form action="{{ route('lessons.retest', [$test_result->test->slug]) }}"
                                                    method="post"> --}}
                                                    <form action="{{ route('lessons.retest', [$test_result->test->first()->test->slug]) }}"
                                                        method="post">
                                        ...
                                        <form action="{{ route('lessons.test', [$lesson->slug]) }}" method="post"
                                            ...
                                            @foreach ($lesson->questions as $question)
                                                ...
                                                @else
                                                    @switch($question->question_type)
                                                            @case("paragraph")
                                                            <textarea type="text" class="form-control editor" placeholder="paragraph" style="height:100px" name="questions[{{ $question->id }}]"></textarea>
                                                            @break
                                                            @case("short_answer")
                                                            <input type="text" class="form-control" placeholder="short_answer" name="questions[{{ $question->id }}]" />
                                                            @break
                                                            @case("date")
                                                            <input type="text" class="form-control datepicker" placeholder="date" name="questions[{{ $question->id }}]" />
                                                            @break
                                                            @case("time")
                                                            <input type="text" class="form-control timepicker"  placeholder="time" name="questions[{{ $question->id }}]" />
                                                            @break
                                                            @case("file_upload")
                                                            <div style="margin-top: 20px">
                                                                <label> @lang('labels.frontend.course.or_upload')</label>
    ...
    <div class="sidebar">
        <nav class="sidebar-nav">
            ...
            <ul class="nav" style="    margin-top: 37px;">
                ...
                {{--  --}}
                {{-- @foreach ($lessonCourse->tests()->get() as $key => $item)                                      
        ...
        @if (count($blogs) > 0)
            ...
            <li
            style="background: #e4e5e6;margin-top: 20px"
            ...
            <ul class="nav-dropdown-items">
                <li>
                    <a class="nav-link "
                        href="{{ route('courses.blogs', ['slug' => $item->slug, 'course_id' => $lessonCourse->id]) }}">{{ $item->title }}</a>
                </li>
            </ul>
            @endforeach
            </li>
        ...
        @if (count($impactMeasurments) > 0)
            ...
            @foreach ($impactMeasurments as $item)
                ...
                <ul class="nav-dropdown-items">
                    <li>
                        <a class="nav-link "
                        href="{{ route('courses.impacts', ['id' => $item->id, 'course_id' => $lessonCourse->id]) }}">@if (Lang::locale() == 'en'){{ $item->name ? $item->name : $item->name_ar }}@else {{ $item->name_ar ? $item->name_ar : $item->name }} @endif</a>
                    </li>
                </ul>
            @endforeach
            </li>
        ...
        @foreach ($programRecommendations as $item)
            ...
            <ul class="nav-dropdown-items">
                <li>
                    <a class="nav-link "
                    href="{{ route('courses.programRecommendations', ['id' => $item->id, 'course_id' => $lessonCourse->id]) }}">@if (Lang::locale() == 'en'){{ $item->name ? $item->name : $item->name_ar }}@else {{ $item->name_ar ? $item->name_ar : $item->name }} @endif</a>
                </li>
            </ul>
        @endforeach
        </li>
        ...
        <div class="couse-feature ul-li-block">
            <ul>
                <li>@lang('labels.frontend.course.chapters')
                    <span> {{ $lessonCourse->chapterCount() }} </span>
                </li>
                <li>@lang('labels.frontend.course.category') <span><a
                            href="{{ route('courses.category', ['category' => $lessonCourse->category->slug]) }}"
                            target="_blank">{{ $lessonCourse->category->name }}</a> </span></li>
                <li>@lang('labels.frontend.course.author') <span>

                        @foreach ($lessonCourse->teachers as $key => $teacher)
                            @php $key++ @endphp
                            <a href="{{ route('teachers.show', ['id' => $teacher->id]) }}" target="_blank">
                                {{ $teacher->full_name }}@if ($key < count($lessonCourse->teachers)), @endif
                            </a>
                        @endforeach
                    </span>
                </li>
                <li>@lang('labels.frontend.course.progress') <span> <b>
                            {{ $lessonCourse->progress() }}
