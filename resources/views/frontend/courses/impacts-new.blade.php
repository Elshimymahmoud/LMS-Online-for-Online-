...
@section('content')
    ...
    if (
        ...
        if ($course->free != 1) {
            $courseLocId = isset($course_location_id)
                ? $course_location_id
                : auth()
                    ->user()
                    ->courseLoc($course->id)
                    ->latest('pivot_created_at')
                    ->first()->pivot->course_location_id;
                                ...
                                <div class="pull-right col-md-3 col-xs-6  ">
                                    <div class="progress">
                                        <div class="progress-bar "
                                            ...
                                            <span
                                                class="color-primary text-color">{{ $course->progress() ? $course->progress() : 0 }}%</span>
                                        </div>
                    ...
                    <div class="container">
                        ...
                        <div class="col-sm-12 col-md-9  benefit wow fadeInUp ptb-50 course-content mt-0">
                            ...
                            <div class="course-details-item border-bottom-0 mb-0" style="background-color:unset">
                                ...
                                @if (($logged_in_user->hasRole('student') ||  $ImpactMeasurement->type == 'student') || ($logged_in_user->hasRole('teacher') || $ImpactMeasurement->type == 'teacher'))
                                    <div class="course-single-text">
                                        ...
                                        <div class="course-title mt10 headline relative-position">
                                            <h3 style="margin: 30px 0">
                                                <b>
                                                    @if (Lang::locale() == 'en')
                                                        {{ $ImpactMeasurement->name ? $ImpactMeasurement->name : $ImpactMeasurement->name_ar }}
                                                    @else
                                                        {{ $ImpactMeasurement->name_ar ? $ImpactMeasurement->name_ar : $ImpactMeasurement->name }}
                                                    @endif
                                                </b>
                                            </h3>
                                        </div>
                                        ...
                                        <form method="POST" enctype="multipart/form-data"
                                            ...
                                            <input type="hidden" name="rate_ids[]" value="{{ $ImpactMeasurement->id }}">
                                            @php
                                                $quest_count = 0;
                                                
                                                $userAnswers = $ImpactMeasurement->getUserRate(null, $course->id);
                                                
                                                $test_result = App\Models\Result::where('course_forms_id', $ImpactMeasurement->course()->first()->pivot->id)
                                                    ->where('user_id', \Auth::id())
                                                    ->first();
                                            @endphp
                                            <div class="">
                                                {{-- @dd($impactMeasurments) --}}
                                                @foreach ($impactMeasurments as $impact)
                                                    @php
                                                    ...
                                                    @foreach ($impact->questions as $question)
                                                        ...
                                                        {{-- <textarea name="questionAnswer-{{ $question->id }}" id=""
                                                            ...
                                                            {{ count($impact->getUserAnswers($course->id))>0?$impact->getUserAnswers($course->id)->where('question_id', $question->id)->first()->answer:'' }}
                                                        </textarea> --}}
                                                        ...
                                                        <ul class="options-list pl-4">
                                                            ...
                                                            @elseif (sizeof($question->options) && $question->question_type == 'drop_down')
                                                                <select name="{{ $question->id }}-options"
                                                                    ...
                                                                    @foreach ($question->options as $option)
                                                                        <option
                                                                            @if ($option->answered(@$test_result->id) != null) selected @endif
                                                                            value="{{ $option->id }}">
                                                                            {{ $option->option_text }}
                                                                        </option>
                                                                    @endforeach
