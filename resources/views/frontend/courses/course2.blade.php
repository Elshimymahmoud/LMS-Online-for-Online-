                    ...
                    <div class="col-xs-10 col-md-5 mt-min-36  mb-36 benefit wow fadeInUp ptb-16 course-content d-grid ">
                        ...
                        <ul class="course-det-list mt-18">
                            ...
                            <li>
                                ...
                                foreach ($course->chapters()->whereIn('id',$lastCourseLocationsChaptersIds)->orderBy('sequence', 'asc')->get() as $chapter)
                                    {
                                        $countLessons+=count($chapter->lessons);
                                    }
                                    foreach ($course->courseTimeline as $item)
                                {
                                    if($item->model_type=='App\Models\Lesson'){
                                       $lessonOfFirstLocation= $item->model?$item->model->whereHas('courseLocations', function ($qq) use ($last) {
    ...
    <section class="row the-product details" id="product">
        <div class="container">
            <div class="row benefit-notes">
                ...
                <div class="  col-md-6  benefit wow fadeInUp  course-content mt-0" style="text-align: justify;
                    padding-top: 50px;">
                ...
                <div class="  col-md-6  benefit wow fadeInUp ptb-50 course-content course-curriclum mt-0">
                    <div class="course-curriclum">
                        ...
                        <div id="accordion" class="curriclum-content">
                            ...
                            @foreach ($course->chapters()->whereIn('id',$lastCourseLocationsChaptersIds)->orderBy('sequence', 'asc')->get()
                                as $key => $chapter)
                                <div class="card">
                                    ...
                                    <div id="collapse-{{ $key }}"
                                        ...
                                        <div class="card-body">
                                            ...
                                            if($item->model_type=='App\Models\Forms'){
                                                    $testsOfFirstLocation= $item->model?$item->model->whereHas('courseLocations', function ($qq) use ($last) {
                                                    $qq->where('lesson_course_location.course_location_id', $last->id)->where('lesson_course_location.model_type','App\Models\Forms');
                                                })->get()->pluck('id')->toArray():[];
                                            }
                                                $lessonsChapter=$chapter->lessons;

                                                @endphp
                                                @if (@$item->model->chapter_id == $chapter->id && $item->model_type=='App\Models\Lesson' &&in_array($item->model->id,$lessonOfFirstLocation))
                                                    <div class=" mb-7 d-flex">
                                                        <i class=" mb-14 "> <img class="w-24"
                                                                src="{{ asset('iv') }}/icons/document.png" alt=""></i>
                                                        <p class="display-content">
                                                            {{ session('locale') == 'ar' ? $item->model->title_ar : $item->model->title }}
                                                        </p>
                                                    </div>
                                                @endif
                                        @if (@$item->model->chapter_id == $chapter->id &&$item->model_type=='App\Models\Forms' &&in_array($item->model->id,$testsOfFirstLocation))
                                        <div class=" mb-7 d-flex">
                                            <i class=" mb-14 "> <img class="w-24"
                                                    src="{{ asset('iv') }}/icons/document.png" alt=""></i>
                                            <p class="display-content">
