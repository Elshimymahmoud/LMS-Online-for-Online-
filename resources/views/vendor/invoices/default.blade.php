...
{{--            @dd($invoice->currency)--}}
            <tr>
                ...
                <td class="text-right">{{ $item->get('totalPrice') }}
                    {{ $invoice->currency }}
                    {{-- {{$item->get('currency')}} --}}

                    {{-- {{ $invoice->formatCurrency()->symbol }} --}}
                </td>
                <td class="text-right">{{ $item->get('totalPrice') }}
                    {{ $invoice->currency }}
                    {{-- {{$item->get('currency')}} --}}
    ...
    <div style="clear:both; position:relative;">
        ...
        <div style="margin-left: 300pt;">
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    <td><b>Subtotal</b></td>
                    <td class="text-right">{{ $invoice->subTotalPriceFormatted() }}
                        {{count($invoice->items)>0?$invoice->currency:$invoice->formatCurrency()->symbol}}
                        {{-- {{ $invoice->formatCurrency()->symbol }} --}}
                    </td>
                </tr>
                @if($invoice->discount != null)
                    <tr>
                        <td><b> - Discount</b></td>
                        <td class="text-right">{{$invoice->discount }}
                        {{count($invoice->items)>0?$invoice->currency:$invoice->formatCurrency()->symbol}}
                ...
                <tr>
                    ...
                    <td><b>TOTAL</b></td>
                    <td class="text-right"><b>{{ $invoice->total }}
                        {{ count($invoice->items) > 0 ? $invoice->currency : $invoice->formatCurrency()->symbol }}
