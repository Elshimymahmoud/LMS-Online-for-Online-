    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-right">
                @if(request('group_id'))
                    <a href="{{ route('admin.groups.rates.attach', ['group' => request('group_id')]) }}"
                ...
                @else
                    <a href="{{ route('admin.groups.rates.create') }}"
                       class="btn btn-success"><i class="icon-plus" title="@lang('strings.backend.general.app_add_new')"></i></a>
                @endif
            </div>
        </div>
                ...
                <thead>
                    <tr>
                        ...
                        <th>@lang('labels.backend.general-titles.title')</th>
                            ...
                            @endif
                        <th>@lang('labels.backend.rates.fields.type')</th>
...
@push('after-scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            var route = '{{ route('admin.group.get_rates') }}';
            ...
            @php
                ...
                if (request('group_id') && request('user_id')) {
                    ...
                    $route = route('admin.group.get_rates', ['group_id' => $group_id, 'user_id' => $user_id, 'show_deleted' => $show_deleted]);
                }
                elseif (request('group_id')) {
                    $group_id = request('group_id');
                    $route = route('admin.group.get_rates', ['group_id' => $group_id, 'show_deleted' => $show_deleted]);
                }
                elseif (request('user_id')) {
                    $user_id = request('user_id');
                    $route = route('admin.group.get_rates', ['user_id' => $user_id, 'show_deleted' => $show_deleted]);
                }
                else {
                    $route = route('admin.group.get_rates', ['show_deleted' => $show_deleted]);
                }
            @endphp
            route = '{{ $route }}';
            //remove &amp; from the url
            ...
            $('#myTable').DataTable({
                ...
                columns: [
                    ...
                    @if(!request('group_id'))
                    {data: "courseGroup", name: "courseGroup"},
                        @endif
