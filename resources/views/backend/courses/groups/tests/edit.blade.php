...
@section('content')
    ...
    {!! Form::model($test, ['method' => 'PUT', 'route' => ['admin.courses.groups.tests.update', "id"=> $test->id],'files' => true]) !!}
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-right">
                <a href="{{  url()->previous() }}" class="btn btn-success">@lang('labels.backend.tests.view')</a>
            </div>
        </div>
        <div class="card-body">
            <div class="row">
            ...
            @if (is_object($test->questions) && sizeof($test->questions) > 0)
                ...
                <input type="hidden" value="{{ $question->id }}" name="q[{{$key}}][id]">
                    <div class="row"
                        style="background:#3bcfcb;margin: 1px;padding-top: 10px;border-radius:5px;position:relative; margin-bottom:1.75rem">
...
{{--                        </div>--}}
                        ...
                        <div class="col-12 form-group form-body">
                            <div class="row">
                                <label style="width:fit-content" for="question_image">
                                    {{ trans('labels.backend.questions.fields.question_image') }}</label>

                                <input class="form-control col-3 " type="file"  id="question_image"
                                       name="q[{{$key}}][image]" accept="image/*">
                            </div>
                            <br>

                            @if($question->question_image)
                                <img src="{{ asset('storage/questions/' . $question->question_image) }}"
                                     alt="Question Image" style="max-width: 80%;height: 15rem;">
                            @endif
                        </div>
                    </div>
...
@push('after-scripts')
  ...
  $('#course_id').on('change', function(e) {
        ...
        $(".add_question").click(function(){
            ...
            new_ques='<div class="row"  style="background:#3bcfcb;margin: 1px;padding-top: 10px;border-radius:5px;' +
