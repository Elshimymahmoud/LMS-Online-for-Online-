        ...
        <div class="card-header">
            ...
            @can('test_create') 
                <div class="float-right">
                    @if(request('group_id'))
                        <a href="{{ route('admin.courses.groups.tests2.atttach', ['group'=>request('group_id')]) }}"
                    ...
                    @else
                        <a href="{{ route('admin.courses.groups.tests2.create') }}"
                           class="btn btn-success"><i class="icon-plus" title="@lang('strings.backend.general.app_add_new')"></i></a>
                    @endif
                </div>
        ...
        <div class="card-body table-responsive">
            ...
            <table id="myTable"
                ...
                <tr>
                    ...
                    <th>@lang('labels.backend.tests.fields.number_questions')</th>
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.courses.groups.tests2.get_data')}}';

            @php
                $show_deleted = (request('show_deleted') == 1) ? 1 : 0;
                if(request('group_id')){
                    $group_id = request('group_id');
                    $route = route('admin.courses.groups.tests2.get_data',['show_deleted' => $show_deleted,'group_id' => $group_id]);
                }
                else{
                    $route = route('admin.courses.groups.tests2.get_data',['show_deleted' => $show_deleted]);
                }
            @endphp
