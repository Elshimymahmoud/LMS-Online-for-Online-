    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-right">
                @if(request('group_id'))
                    <a href="{{ route('admin.groups.rec.attach', ['group' => request('group_id')]) }}"
                ...
                @else
                    <a href="{{ route('admin.groups.rec.create') }}"
                       class="btn btn-success"><i class="icon-plus" title="@lang('strings.backend.general.app_add_new')"></i></a>
                @endif
            </div>
        </div>
...
@push('after-scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            var route = '{{ route('admin.group.get_rec') }}';

            @php
                $show_deleted = request('show_deleted') == 1 ? 1 : 0;
                if (request('group_id')) {
                    $group_id = request('group_id');
                    $route = route('admin.group.get_rec', ['group_id' => $group_id, 'show_deleted' => $show_deleted]);
                } else {
                    $route = route('admin.group.get_rec', ['show_deleted' => $show_deleted]);
                }
            @endphp
            route = '{{ $route }}';
            //remove &amp; from the url
