...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="page-title float-left mb-0">@lang('menus.backend.sidebar.courses.courses_include')</h3>
            @can('course_create')
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            ...
            var route = '{{route('admin.courses.get_data1')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.courses.get_data1',['show_deleted' => 1])}}';
            @endif

            @if(request('teacher_id') != "")
                route = '{{route('admin.courses.get_data1',['teacher_id' => request('teacher_id')])}}';
            @endif

            @if(request('cat_id') != "")
                route = '{{route('admin.courses.get_data1',['cat_id' => request('cat_id')])}}';
            @endif
