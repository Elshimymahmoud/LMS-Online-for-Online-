...
@section('content')
{!! Form::open(['method' => 'PUT', 'route' => ['admin.trainingDataQuestions.update', 'id'=>$trainingDataQuestion], 'files' => true,]) !!}
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-12 form-group">
                ...
                'control-label']) !!}
                {!! Form::text('question', $trainingDataQuestion->question, ['class' => 'form-control ', 'placeholder' => '', 'required'
                => '']) !!}
        ...
        <div class="row">
            <div class="col-12 form-group">
                ...
                'control-label']) !!}
                {!! Form::text('question_ar', $trainingDataQuestion->question_ar, ['class' => 'form-control ', 'placeholder' => '',
                'required' => '']) !!}
                <p class="help-block"></p>
                @if($errors->has('question'))
                <p class="help-block">
        ...
        <div class="row form-group">
            ...
            <div class="col-md-12">
                <select name="type" onchange="controlChoose(event)"  id="questionType" class="form-control">
                    @if($trainingDataQuestion->type=='text')
                    ...
                    @elseif($trainingDataQuestion->type=='choose')
                    <option value="choose">{{__('labels.backend.trainingData.choose')}}</option>
                    <option value="text">{{__('labels.backend.trainingData.text')}}</option>
                    <option value="file">{{__('labels.backend.trainingData.file')}}</option>
                    @else
        ...
        <div class="row form-group" id="chooseNumberDiv" @if($trainingDataQuestion->type!='choose')style="display: none" @endif>
            {{ html()->label(__('labels.backend.trainingData.fields.answer_option_number'))->class('col-md-2
            form-control-label')->for('impact_title_ar') }}

            <div class="col-md-12">
                <input type="number" value="{{count($trainingDataQuestion->options)}}" onchange="setOptions(event)" name="chooseNumber" class="form-control"
                    id="chooseNumber">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-12 form-group">
                    ...
                    {!! Form::text('option_text_' . $question, isset($trainingDataQuestion->options[$question-1])?$trainingDataQuestion->options[$question-1]->option_text:'', ['class' => 'form-control ', 'rows'
                    => 3]) !!}
                    <p class="help-block"></p>
