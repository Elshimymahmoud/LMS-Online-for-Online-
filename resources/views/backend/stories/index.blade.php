            ...
            <table id="myTable"
                ...
                <tr>
                    ...
                    @endcan
                        ...
                        <th>@lang('labels.backend.stories.fields.title')</th>
                    <th>@lang('labels.backend.stories.fields.title_ar')</th>
                     @if( request('show_deleted') == 1 )
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.stories.get_data')}}';
            ...
            @can('question_delete')
            @if(request('show_deleted') != 1)
            $('.actions').html('<a href="' + '{{ route('admin.stories.mass_destroy') }}' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');
            @endif
            @endcan
        ...
        $(document).on('click', '.switch-input', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.stories.status') }}",
                data: {
                    _token:'{{ csrf_token() }}',
                    id: id,
                },
