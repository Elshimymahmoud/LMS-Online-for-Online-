...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row @if(!isset($courses_clint)) collapse @endif" id="createCat">
                <div class="col-12">
                    @if(isset($courses_clint))
                        {!! Form::model($courses_clint, ['method' => 'PUT', 'route' => ['admin.courses_clints.update', $courses_clint->id], 'files' => true,]) !!}
                    @else
                        {!! Form::open(['method' => 'POST', 'route' => ['admin.courses_clints.store'], 'files' => true,]) !!}
                    @endif
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-12 form-group mb-0">
                            ...
                            @if(isset($courses_clint) && ($courses_clint->logo != null))
                                {!! Form::file('logo', ['class' => 'form-control w-75 d-inline-block', 'placeholder' => '', 'accept' => 'image/jpeg,image/gif,image/png']) !!}
                                    <img class="d-inline-block" src="{{asset('storage/uploads/'.$courses_clint->logo)}}" height="38px">
                            @else
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.courses_clints.get_data')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.courses_clints.get_data',['show_deleted' => 1])}}';
            @endif
            ...
            $('.actions').html('<a href="' + '{{ route('admin.courses_clints.mass_destroy') }}' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');
        ...
        $(document).on('click', '.switch-input', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.courses_clints.status') }}",
                data: {
                    _token:'{{ csrf_token() }}',
                    id: id,
                },
