                        ...
                        @if (count($tests) > 0)
                            @foreach ($tests as $test)
                                <tr data-entry-id="{{ $test->id }}">
                                    <td>{{ $test->getcourseByFormId()?$test->getcourseByFormId()->title:''}}</td>
                                    <td>{{ $test->title }}</td>
                                    <td>
                                    ...
                                    @else
                                        <td>
                                            @can('test_view')
                                                <a href="{{ route('admin.forms.show',[$test->id,'form_type'=>$test->form_type,'course_id'=>$test->getcourseByFormId()?$test->getcourseByFormId()->id:0]) }}"
                                                   class="btn btn-xs mb-2 btn-primary">
                                                    <i class="icon-eye"></i>
                                                </a>
                                            @endcan
                                            @can('test_edit')
                                                <a href="{{ route('admin.forms.edit',[$test->id,'form_type'=>$test->form_type,'course_id'=>$test->getcourseByFormId()?$test->getcourseByFormId()->id:0]) }}"
                                            ...
                                            @can('test_delete')
                                                    ...
                                                    <a data-method="delete" data-trans-button-cancel="Cancel"
                                                       ...
                                                       onclick="$(this).find('form').submit();">
                                                        ...
                                                        <form action="{{route('admin.forms.destroy',['test'=> $test->id,'form_type'=>$test->form_type])}}"
                                                              method="POST" name="delete_item" style="display:none">
                                                            @csrf
                                                            {{method_field('DELETE')}}
                                                        </form>
